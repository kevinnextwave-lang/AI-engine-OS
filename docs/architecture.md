...
- **Safety** (`crawler/safety.py`): http/https only; blocked hostnames (`localhost`, `*.internal`, metadata names); DNS resolved and every address must be public (no private/loopback/link-local/multicast/CGNAT/metadata ranges, IPv4-mapped IPv6 unwrapped). Every redirect hop is re-checked, and the fetcher connects to the exact address the check validated (original hostname kept as Host header and SNI), so a DNS rebind between check and connect cannot reach a private address. A blocked root fails the job.
...
### Citation Intelligence frontend (`apps/web`)

Navigation group **AI Intelligence** → Overview, AI Responses, Citations, Sources, Claims, Citation Gaps (`src/app/(app)/app/ai-intelligence/*`). Same layering as the other sections: `src/lib/intelligence/` (view-model types, pure mappers, labelled sample data, `useIntelligenceData` loading graph overview, sources, claims, gaps, gap summary, citations, prompt sets and provider status for one project + 30/90/180-day window); `src/components/intelligence/` renders only. Overview: five metric tiles, top-cited-sources bars (brand / competitor / other segments), "Where competitors are winning" opportunity cards from the gap summary, and the **AI Search Graph** — a simplified deterministic inline-SVG layout (brand centre, competitors ring, prompts left, sources right; mentioned / cited / associated / competes-with edges; only the strongest citation edges drawn) with competitor (client), source-type and AI-engine (server, `/graph/overview?provider=&source_type=`) filters plus the header date range. Source drawer derives pages, engines, brands, competitors, weekly trend and prompts from `/projects/{id}/citations?source_domain_id=`; gap drawer shows opportunity components, competitor vs brand bars, evidence, prompts, a neutral recommendation with an explicit no-manipulation note, and status/note updates. Empty state: "Run more AI searches to build your citation intelligence graph." with *Run AI Search Analysis*. Backend support added for this section: `GET /projects/{id}/citations` (filterable list with prompt, engine and relationships), `provider`/`source_type` filters and brand/competitor citation counts on `/graph/overview`.

## Recommendation Engine (Milestone 4E)

`apps/api/app/recommendations/`: turns citation gaps into reviewable `recommendations` rows (types citation / content / entity / technical / authority / reputation; v1 generates the first two). Pure rules (`rules.py`) decide recommendability, priority (opportunity × confidence × business relevance, with competitor-gap and sample-size gates for `critical`) and the five-part explanation; the engine (`engine.py`) gathers evidence (commercial prompts per source, research-type citations), upserts by a stable `source_key` so review status survives regeneration, and only recommends "Create original research" when competitors are cited for research, the brand is not, and the prompts are commercial. Human review via approve / dismiss / start (+ PATCH for reviewing / completed / reopen) with a validated transition table; nothing is executed automatically. Methodology in `docs/recommendations.md`.

## Competitor intelligence data model (Milestone 5A)

The existing `competitors` table is extended, not replaced (`hostname` is the competitor's domain; new `normalized_domain`, `normalized_name`, `description`, `source` manual / discovered / imported / ai_detected, `status` active / ignored / archived, `confidence` high / medium / low; unique per project on normalised domain and on normalised name). New tables: `competitor_aliases` (company names, product names, abbreviations, alternate spellings; unique per competitor on the alias text), `competitor_domains` (primary / product / support / blog / community / other, one primary — created automatically from the website URL), `competitor_products`. `app/competitors/service.py` owns all writes with duplicate handling across the project: same normalised domain or any competitor domain, same normalised name, a name that is already another competitor's alias or product, and the project's own domains are all rejected with 409. Routes: `GET/POST /projects/{id}/competitors`, `GET/PATCH/DELETE /competitors/{id}`, and nested `/aliases`, `/domains`, `/products`; competitor routes derive the project from the row so other tenants get 404; DATA_MANAGE for writes. Migration `6053d5ad9c4e` backfills existing rows (normalised fields, a primary domain row each). Competitor AI visibility, mentions, citations and claims continue to come from the visibility, intelligence and graph services keyed by competitor id/name.

## AI Competitor Discovery (Milestone 5B)

`apps/api/app/discovery/` combines three sources — deterministic extraction from stored AI responses (list items, "alternatives to", "X vs Y", "competitors such as" phrasing, with cited-URL hosts as domain evidence), organisation entities from the customer's own site (a hint that only enriches names seen elsewhere) and an AI-assisted pass through the configured provider returning strict JSON validated by Pydantic — into `competitor_candidates` (unique per project on normalised name; status new / reviewing / accepted / rejected). The brand, own domains and configured competitors with aliases, products, domains and editions are excluded; a name seen in one response and nowhere else is never written. Confidence is an explained weighted sum (frequency, relevance, domain confidence, competitor language, cross-provider occurrence) stored with the evidence. Nothing becomes a competitor automatically: `POST /competitor-candidates/{id}/accept` creates one through the 5A service (source `discovered`), `/reject` keeps the name out; re-runs update evidence but never reviewed status. Routes under `/projects/{id}/competitor-candidates[/discover]` and `/competitor-candidates/{id}`. See `docs/competitor-discovery.md`.

## Competitive AI Visibility (Milestone 5C)

`apps/api/app/competitive/` compares the brand with every configured competitor over the same eligible responses the AI Visibility Score uses: mention share, recommendation share, average position, citation share (own-site hosts and `citation_entities` evidence, now carried per entity on each observation), sentiment and prompt coverage, plus a Competitive Visibility Score (our own weighted composite — mention 30, recommendation 25, position 15, citation 15, sentiment 15 — withheld below 5 responses and rounded by sufficiency). Advantages are `competitor − brand` per score and component, flagged material only at 20+ responses and ≥ 10 points; rankings only at 20+ responses; per-prompt leaders only at 5+ responses. Read-only routes `GET /projects/{id}/competitive-visibility[/trends|/prompts|/engines]` each carry `data_quality` (sample size, prompt count, provider count, date range, confidence). See `docs/competitive-visibility.md`.

## Why Competitors Win (Milestone 5D)

`apps/api/app/insights/` turns the response/citation graph into `competitive_insights` — evidence-backed, explicitly non-causal observations per competitor (types: content / citation / entity / evidence / positioning / coverage advantage; confidence and impact high/medium/low). Pure analyzers with minimum-evidence bars (thin data produces nothing; < 10 eligible responses produces nothing at all) compare citation footprints (unique citing domains by type, authority), cited content categories, prompt coverage, positioning (5C), research citations and specific claims; the entity analyzer only reports brand-side structured-data gaps as potential contributing factors because competitor sites are not crawled. Rows are unique per (project, competitor, type); re-analysis upserts and removes stale insights. Routes: `GET/POST /projects/{id}/competitive-insights[/analyze]`, `GET /competitors/{id}/insights`. See `docs/competitive-insights.md`.

## Competitive Content Gap Engine (Milestone 5E)

`apps/api/app/content_gaps/` compares, per prompt-topic, competitor visibility in parsed AI responses with the crawled site's actual coverage (keyword matching over page URL/title/meta, word-count thinness, page categories) and persists `content_gaps` (types missing_topic / weak_topic / missing_comparison / missing_use_case / missing_faq / missing_evidence / missing_product_detail; unique per project+topic+type). Gaps require ≥ 5 responses, a competitor in ≥ 40% of them and a ≥ 20-point lead over the brand — content is never recommended blindly. Opportunity Score (0–100) weighs competitor advantage 30, prompt frequency 25, commercial relevance 20, coverage deficit 15, evidence availability 10, with components stored in the evidence. Re-analysis upserts; review status/note survive; stale `new` rows are removed. Routes: `GET/POST /projects/{id}/content-gaps[/analyze]`, `GET/PATCH /content-gaps/{id}`. See `docs/content-gaps.md`.

## Competitive AI Alerts (Milestone 5F)

`apps/api/app/alerts/` compares two adjacent periods of the 5C observations (raw shares, not presentation-rounded) and detects novelty across discovery, citations, claims and content gaps, persisting `competitive_alerts` (types: competitor_overtakes_brand, visibility_drop, competitor_visibility_jump, new_competitor, new_citation_source, citation_gap_increase, new_competitor_claim, content_gap; severity critical/high/medium/low; status new/read/dismissed). Every rule has a configurable threshold (`AlertThresholds`, overridable per detect call and echoed in evidence) and a per-period minimum sample, so insignificant changes never alert; severity escalates at 2× threshold. Evidence always carries previous/current measurements, both date ranges, affected prompts/providers and a sample-based confidence. Dedup: unique (project, dedup_key = type:subject:fingerprint) — re-detection updates evidence, never status (dismissed stays dismissed). `notifications.py` defines the future email/Slack/Teams/webhook seam (protocol + dispatcher, log-only today). Routes: `GET/POST /projects/{id}/competitive-alerts[/detect]`, `GET/PATCH /competitive-alerts/{id}`. See `docs/competitive-alerts.md`.

## Agent Framework (Milestone 6A)

`apps/api/app/agents/` is the reusable agent architecture (no concrete agents ship with it): `Agent` base class with provider-neutral `ModelPreferences` (preferred/fallback model, max tokens, per-run cost ceiling enforced by `AgentLLM`), `AgentContext` built by targeted retrieval (per-category row budgets, never the whole database), a controlled `ToolBox` (fixed project-scoped queries with pydantic-validated args, row/text caps, ownership checks, logged usage — no SQL from LLMs, ever), prompt assembly with strict trust separation (crawled/AI content is fenced untrusted data, never system instructions, markers neutralized), and an orchestrator owning `agent_runs`/`agent_actions`: queued runs execute on the Celery `agents` queue, medium/high-risk proposed actions are forced through human approval (pending → approve/reject with the acting user recorded; runs settle from awaiting_approval to completed), cancellation wins over results, failures and budget breaches are recorded with tokens, cost and timing. Routes: `POST /projects/{id}/agents/{name}/run`, `GET /projects/{id}/agent-runs`, `GET /agent-runs/{id}[/actions]`, `POST /agent-runs/{id}/cancel`, `POST /agent-actions/{id}/approve|reject`. See `docs/agent-framework.md`.

## Research Agent (Milestone 6B)

`apps/api/app/agents/research.py` is the first agent on the framework, registered as `research` and run via the generic agent endpoint. It is deterministic by construction — every number comes from a controlled tool call (competitive visibility, citation/content gaps, insights, candidates, entities, pages, responses, latest SEO audit, claims; five tools added to the 6A registry), so nothing can be hallucinated. Three-step process: identify significant gaps (material 5C leads, ≥ 60-opportunity 4C/5E gaps excluding competitor-owned sites, weak Organization markup, unreviewed 5B candidates, poor technical-SEO audits, repeated competitor claims), validate evidence (supporting ids and measurements per finding, confidence capped low under 10 responses), prioritize (relevance 25 / visibility 25 / competitor advantage 20 / evidence 20 / effort 10; top 3–7). Every finding separates observed / inference / recommendation statements; opportunities carry why_now, recommended_action and expected area of impact; proposed actions are suggestions (content/entity work at medium risk → forced human approval). See `docs/research-agent.md`.

## Content Strategy Agent (Milestone 6C)

`apps/api/app/agents/content_strategy.py` (registered as `content-strategy`, run via the generic agent endpoint) turns high-opportunity content gaps into `content_briefs` — what to create, why, for whom, targeting which real prompts, against which competitors, with which evidence — never the content itself. Deterministic like the research agent: every value comes from a controlled tool call, evidence requirements describe real material to collect with a standing do-not-fabricate rule, candidate citation sources come only from observed citation gaps (competitor-owned sites excluded), and every objective states that no ranking outcome is guaranteed. Briefs persist through `save_content_brief`, the framework's only write tool (schema-validated, project-scoped, prompt/competitor ids verified, draft on insert, review status preserved on re-save; unique per project+source_key). Lifecycle draft → reviewing → approved → in_progress → completed → archived via `GET/PATCH /content-briefs/{id}` and `GET /projects/{id}/content-briefs`; each run proposes medium-risk review actions so it ends awaiting approval. See `docs/content-strategy-agent.md`.

## GEO Content Optimization Agent (Milestone 6D)

`apps/api/app/agents/content_optimization.py` (registered `content-optimization`) reviews crawled pages for answerability, entity clarity, specificity, evidence quality, structure, justified entity markup and internal linking, and writes `content_optimization_reviews` (unique per project+page) through the validated `save_content_review` write tool; page text arrives via the new `get_page_content` tool and stays untrusted. Every proposal has location / current_text / proposed_text / reason / evidence / confidence with `[FILL: …]` placeholders wherever a real fact is needed — nothing is invented, no ranking is guaranteed, and the original page is never modified. Humans decide each change (accept / edit-with-text / reject) via `POST /content-reviews/{id}/changes/{change_id}`; decisions and review status survive re-runs (change identity = location+current_text hash); accepted changes await a future integration. Routes: `GET /projects/{id}/content-reviews`, `GET/PATCH /content-reviews/{id}`. See `docs/content-optimization-agent.md`.

## Entity Optimization Agent (Milestone 6E)

`apps/api/app/agents/entity_optimization.py` (registered `entity-optimization`) reviews how the organization, products, services and people are represented across crawled entities, structured data, pages and links, and writes `entity_optimization_reviews` (unique per project + review_key) through the validated `save_entity_review` tool. Checks: Organization (name/description/url/logo/contact/sameAs/location, founding info only-if-known, industry; absence itself is a finding), Products (description, features, audience, pricing, URL, brand/publisher link to the organization; product-like pages without Product entities), Services (definition, audience, areaServed, provider), People (existing Person entities only — never invented credentials), and cross-page consistency (conflicting names/descriptions/sameAs with page URLs → "resolve conflicting company information"). Every recommendation notes that engine interpretation is not guaranteed; all modifications require human approval (review status workflow + medium-risk run actions). Routes: `GET /projects/{id}/entity-reviews`, `GET/PATCH /entity-reviews/{id}`. See `docs/entity-optimization-agent.md`.
